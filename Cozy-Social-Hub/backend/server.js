...
/**
 * API 端點：獲取目前活動中的戶資料
 * 用途：供展示螢幕（display.html）獲取即時的在線用戶和話題資料
 * 流程：
...
app.post('/api/user-leave', async (req, res) => {
    try {
        ...
        // 處理話題相關邏輯
        if (leavingUser.topic) {
            // 如果是話題主持人
            const topic = topicData[cardId];
            if (topic) {
                // 關閉話題報名
                topic.status.registrationStatus.isOpen = false;
                topic.status.hostLeft = true;
                topic.status.hostLeftAt = new Date().toISOString();

                // 釋放報名點
                delete topic.registrationPoint;

                // 更新主持人的歷史記錄
                const hostHistory = personalData[cardId].history.createdTopics.find(
                    t => t.topicId === cardId
                );
                if (hostHistory) {
                    hostHistory.endedAt = new Date().toISOString();
                    hostHistory.endReason = 'host_left';
                }

                // 通知所有參與者話題已結束
                topic.status.currentAttendees.forEach(attendee => {
                    if (attendee.id !== cardId && personalData[attendee.id]) {
                        const attendeeHistory = personalData[attendee.id].history.joinedTopics.find(
                            t => t.topicId === cardId
                        );
                        if (attendeeHistory) {
                            attendeeHistory.endedAt = new Date().toISOString();
            ...
            // 如果是參與者，更新所有參與的話題
            Object.entries(topicData).forEach(([topicId, topic]) => {
                const attendeeIndex = topic.status.currentAttendees
                    .findIndex(a => a.id === cardId);
                
                if (attendeeIndex !== -1) {
                    // 從參與者列表中移除
                    topic.status.currentAttendees.splice(attendeeIndex, 1);
                    topic.status.attendeeCount--;
                    ...
                    if (joinedTopic) {
                        joinedTopic.leftAt = new Date().toISOString();
        ...
        res.json({
            ...
            data: {
                ...
                affectedTopicId: leavingUser.topic ? cardId : null
            }
        });
