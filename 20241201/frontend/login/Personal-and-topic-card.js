...
const baseUrl = 'https://4ff4-180-217-241-190.ngrok-free.app';
...
document.querySelector('.save-btn').addEventListener('click', async function() {
    ...
    if (topicTitle) {
        ...
        // 發送話題資料到後端
        try {
            const topicResponse = await fetch(`${baseUrl}/api/topic-card`, {
                method: 'POST',
                headers: {
    ...
    // 發送個人資料到後端
    try {
        const response = await fetch(`${baseUrl}/api/personal-card`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function initializePersonalData() {
    try {
        ...
        // 從後端獲取個人資料
        const response = await fetch(`${baseUrl}/api/personal-card/${cardId}`);
